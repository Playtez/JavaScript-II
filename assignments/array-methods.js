...
let fullNames = [];
runners.forEach(function(bibNames) {
    fullNames.push(`${bibNames.first_name} ${bibNames.last_name}`);
});
...
const newArray = runners.map(function(names) {
    return names.first_name.toUpperCase();
});
firstNamesAllCaps.push(newArray);
...
const listOfShirts = runners.filter(function(shirt_L) {
    return shirt_L.shirt_size === "L";
});
runnersLargeSizeShirt.push(listOfShirts);
